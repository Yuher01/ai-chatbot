...
- **New `conversational` route**: Added a dedicated `conversational_response` node to handle non-document queries (greetings, capability questions, chitchat) without going through the RAG pipeline. Previously all queries were routed through retrieval.
- **Intent classification in query analysis**: The `QueryAnalysis` schema now includes a `route` field (`"rag"` or `"conversational"`). The query analysis prompt classifies intent before rewriting, with a RAG-first bias — any query containing a topic/subject routes to RAG even if it starts with filler words like "alright" or "ok".
- **Updated routing logic**: `route_after_rewrite` in `edges.py` now handles three paths: `conversational_response`, `human_input` (unclear query), or fan-out to `process_question` (RAG).
- **Graph state**: Added `route: str = "rag"` field to `State` in `graph_state.py`.

**Files changed**: `schemas.py`, `prompts.py`, `nodes.py`, `edges.py`, `graph.py`, `graph_state.py`

### Document-Aware Question Suggestions

- **Random chunk sampling per document**: New `_sample_document_snippets()` helper in `nodes.py` uses Qdrant's `query_points` with `SampleQuery(RANDOM)` and per-document filtering to sample 3 random chunks from each uploaded document.
- **Conversational response uses snippets**: When a user asks "what can I ask?" or similar capability questions, the conversational node retrieves real content snippets and passes them to the prompt so the LLM can suggest specific, content-aware questions (up to 5, at least one per document).
- **Suggestions on failed retrieval**: When the aggregation node detects a "couldn't find information" response, it samples document snippets and appends suggested questions the user could try instead.
- **New prompt**: `get_suggestion_prompt()` instructs the LLM to generate up to 5 specific questions from document snippets.
- **Collection passed through graph**: The Qdrant collection object is now threaded from `rag_system.py` → `create_agent_graph()` → `conversational_response` and `aggregate_responses` nodes.

**Files changed**: `nodes.py`, `prompts.py`, `graph.py`, `rag_system.py`

### Incremental Conversation Summarization

- **Preserves context across turns**: `analyze_chat_and_summarize` now reads the existing `conversation_summary` from state and prepends it to the summarizer input. The LLM produces an updated summary incorporating both old context and new messages, instead of starting from scratch each turn.
- **Increased summary word limit**: Changed from 30-50 words to 80-100 words, giving the summarizer room to retain key topics without significant token cost increase downstream.

**Files changed**: `nodes.py`, `prompts.py`

### RAG Agent Response Guardrails

- **No-answer guardrail**: Updated the RAG agent prompt to strictly respond with a fixed message when no information is found after retry, preventing the LLM from exposing document names, topics, or scope.
- **Source stripping in aggregation**: `aggregate_responses` now strips the `**Sources:**` section if it contains no real file names (e.g., only "Answer 1" references).

**Files changed**: `prompts.py`, `nodes.py`

### Document Source Awareness

- **`get_document_sources` callback**: `RAGSystem.initialize()` now accepts an optional `get_document_sources` callback, wired from `gradio_app.py` to `doc_manager.get_markdown_files`. This provides runtime awareness of uploaded document filenames to the conversational and aggregation nodes.

**Files changed**: `rag_system.py`, `gradio_app.py`, `graph.py`

### Debug Mode

- **LangChain debug logging**: `app.py` now calls `set_debug(True)` to enable verbose LangChain trace output for development.